...
The public artifact is Developer ID-signed and notarized. It is for macOS 14 or later on Apple Silicon, and the link above points to the published GitHub Release asset. The source tree can be built and tested locally, and `make package` creates a local Apple Silicon bundle for evaluation. That local bundle is ad-hoc signed and is not a substitute for the public artifact.
...
**Refresh** in the Analytics window. The feature is released and remains local-only; no analytics history is retained after the in-memory state is discarded. A later failed refresh can leave the last successful in-memory snapshot marked stale, while an analysis with no successful result is Unavailable.
